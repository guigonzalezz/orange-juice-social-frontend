...
const FormAlteraUsuario = ({ handleClose, cargos, usuario }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      await axios.patch(`${process.env.HEROKU_OJ_API_DEV_URL}/usuario/atualizar_perfil/${usuario.id_usuario}`,{
        ...
        if(formik.values.cargo != usuario.id_cargo) {
          await axios.patch(`${process.env.HEROKU_OJ_API_DEV_URL}/usuario/atualizar_cargo/${usuario.id_usuario}?id_cargo=${formik.values.cargo}`)
        }
        Swal.fire({
          ...similarCustomSA,
          icon: 'success',
